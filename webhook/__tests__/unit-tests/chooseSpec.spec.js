...
describe('Test chooseSpec function', () => {
  ...
  it('Test if function runs correctly when specification is "shipping_time"', async () => {
    const addStub = stub().returns();
    const setTitleStub = spy(Card.prototype, 'setTitle');
    ...
    assert.calledOnceWithMatch(setTitleStub, `A T1 Básico é entregue a você entre ${mockMachine.shipping_time.min} e ${mockMachine.shipping_time.max} dias úteis.`);
    assert.calledWith(addStub, new Card({
    ...
    assert.calledWith(addStub, 'Gostaria de saber mais alguma coisa?');
  });
  ...
  it('Test if function runs correctly when specification is "total_amount"', async () => {
    ...
    assert.calledOnceWithMatch(getStub, 'choosemachine-followup');
    assert.calledOnceWithMatch(setTitleStub, `O valor de adesão da T1 Básico é de R$ ${mockMachine.total_amount} e você pode parcelar em até 12x sem juros!`);
    assert.calledWith(addStub, new Card({
      title: `O valor de adesão da T1 Básico é de R$ ${mockMachine.total_amount} e você pode parcelar em até 12x sem juros!`,
      buttonText: 'Peça a sua T1 Básico agora!',
      buttonUrl: 'https://www.ton.com.br/maquininha/t1',
    }));
    assert.calledWith(addStub, 'Te ajudo em algo mais?');
  });
});
